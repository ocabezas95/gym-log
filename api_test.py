...
def test_B1_save_valid_suggestion():
    ...
    with patch("api.load_exercises", return_value=list(EXERCISES)), \
         patch("api.save_exercises") as mock_save:
    ...
    assert saved[0]["suggested_weight"] == 100.0
...
def test_B5_overwrite_existing_suggestion():
    ...
    exercises_with_suggestion = [
        {"id": 1, "name": "Squat", "equipment": "Barbell", "suggested_weight": 80.0},
    ]
    with patch("api.load_exercises", return_value=exercises_with_suggestion), \
         patch("api.save_exercises") as mock_save:
        resp = client.patch("/api/exercises/1/suggestion", json={"suggested_weight": 90.0})
    assert resp.status_code == 200
    assert resp.json()["suggested_weight"] == 90.0
    saved = mock_save.call_args[0][0]
    assert saved[0]["suggested_weight"] == 90.0
